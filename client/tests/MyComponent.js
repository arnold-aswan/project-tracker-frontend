...
describe("AboutUs Component", () => {
  ...
  it("displays mission and vision content", () => {
    ...
    expect(
      getByText(
        "We are on a mission to revolutionize the way teams manage their projects."
      )
    ).toBeInTheDocument();
    expect(
      getByText(
        "We envision a world where every project, no matter the size or complexity, can be efficiently managed and completed on time."
      )
    ).toBeInTheDocument();
  });
  ...
  it('displays "What Sets Us Apart" section content', () => {
    ...
    expect(
      getByText(
        "Our platform is built with a user-centric approach, ensuring an intuitive and user-friendly interface."
      )
    ).toBeInTheDocument();
    expect(
      getByText(
